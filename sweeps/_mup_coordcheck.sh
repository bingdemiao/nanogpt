...
python -c "import cute_prism" 2>/dev/null || uv pip install --no-build-isolation -e "/opt/cute_mma[mup]"
python -c "import mup" 2>/dev/null || uv pip install mup

echo "=== muP coordinate check (activations should be ~flat across width) ==="
