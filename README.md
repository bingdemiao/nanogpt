...
The `nanogpt` env reuses `oft.sqfs` but mounts `$HOME/nanogpt` as `/workspace`. For login-node smoke tests use enroot directly:

```bash
ENROOT_SLURM_HOOK=off enroot start --rw \
    -m $HOME/nanogpt:/workspace \
    -m $HOME/cute_mma:$HOME/cute_mma \
    -m $HOME/.cache:$HOME/.cache \
    -e CUTE_PRISM_CACHE_DIR=/tmp/cute_prism_cache \
    -e NCCL_SOCKET_IFNAME=lo \
    /scratch/cscs/<username>/containers/oft.sqfs \
    bash
...
[`tools/parse_iter_times.py`](https://www.google.com/search?q=tools/parse_iter_times.py); FLOPs are
from [`tools/count_flops.py`](https://www.google.com/search?q=tools/count_flops.py) (the prism
...
* [x] Vanilla 4× run: `nanogpt-vanilla-tinystories` (run log recorded)
* [x] Prism 4× run: `nanogpt-prism-tinystories` (reached iter 45000, val loss 1.3566)
...
prepared in [`sweeps/lr_prism_3x_extend.sh`](https://www.google.com/search?q=sweeps/lr_prism_3x_extend.sh).
...
50 M-param models, see [Datasets section](https://www.google.com/search?q=%23datasets)) or a genuine
...
rejects input_shuffle. Training uses `cublas` per TRAINING_GATED_PRISM.md §9 so this doesn't affect us.
* works with `bf16` + `silu_gate` (our training setup; both forward and
backward verified)

Smallest ablation to test the shuffle (2 jobs, ~5 h each):
[`sweeps/input_shuffle_prism_3x.sh`](https://www.google.com/search?q=sweeps/input_shuffle_prism_3x.sh).
...
each sweep point has its own checkpoint directory and run tracking.

Ready-made launcher: [`sweeps/lr_prism_3x.sh`](https://www.google.com/search?q=sweeps/lr_prism_3x.sh)
...
Configs (all in [`config/`](https://www.google.com/search?q=config/)):
...
# writes ~20 GB to $SCRATCH via the symlinked data/fineweb/).
...
* [ ] `grad_norm` is currently logged as 0 in [train.py:295-323](https://www.google.com/search?q=train.py%23L295-L323) — pre-existing
...
[`sweeps/input_shuffle_prism_3x.sh`](https://www.google.com/search?q=sweeps/input_shuffle_prism_3x.sh).
* [ ] `data/`, `out-*` directories are symlinked to `$SCRATCH`
to keep home directory quota clean. The slurm container
env (`~/.edf/nanogpt.toml`) mounts `$SCRATCH` so they
resolve inside the container — confirmed.

## Sweep scripts

All multi-job launchers live in [`sweeps/`](https://www.google.com/search?q=sweeps/). They use one base
config plus `EXTRA` overrides — no need for N nearly-identical config
files.

| Script | What it does | When to run |
| --- | --- | --- |
| [`sweeps/lr_prism_3x.sh`](https://www.google.com/search?q=sweeps/lr_prism_3x.sh) | Step 5a — 2-D sweep `learning_rate × r_init_scale` on TinyStories prism-3× (11 jobs). **Done.** Best: lr=1.5e-3, r_init=0.5 → val 1.3100. | Already run. |
| [`sweeps/lr_prism_3x_extend.sh`](https://www.google.com/search?q=sweeps/lr_prism_3x_extend.sh) | Step 5a-extend — push prism-3× LR to {2e-3, 3e-3} at r_init=0.5 (2 jobs). The first sweep ended on a still-decreasing trend. | Optional — only if you want to find the actual prism LR peak. |
| [`sweeps/lr_vanilla.sh`](https://www.google.com/search?q=sweeps/lr_vanilla.sh) | Step 5b — vanilla 4× LR sweep on TinyStories at `{3e-4, 1e-3, 1.5e-3, 2e-3}` (4 jobs). | Run before Step 6 — needed for a fair vanilla-vs-prism baseline. |
| [`sweeps/prep_fineweb.sh`](https://www.google.com/search?q=sweeps/prep_fineweb.sh) | One-shot data prep — downloads FineWeb-Edu sample-10BT and tokenizes to `data/fineweb/{train,val}.bin` (~20 GB on scratch). **Done.** | Already run. |
| [`sweeps/fineweb_param_matched.sh`](https://www.google.com/search?q=sweeps/fineweb_param_matched.sh) | Step 6 — submits all four GPT-2-scale runs (vanilla 4× + prism 4×/3×/2×) on FineWeb in parallel. | After Step 5b. Override LR via EXTRA per config when launching (e.g. `EXTRA="--learning_rate=1.5e-3 --min_lr=1.5e-4"` for the prism configs). |
| [`sweeps/measure_flops.sh`](https://www.google.com/search?q=sweeps/measure_flops.sh) | One-shot — runs [`tools/count_flops.py`](https://www.google.com/search?q=tools/count_flops.py) inside the container env to produce the FLOPs comparison table. | Any time the model code changes. ~30 s. |
| [`sweeps/input_shuffle_prism_3x.sh`](https://www.google.com/search?q=sweeps/input_shuffle_prism_3x.sh) | 2 jobs at the tuned recipe: prism-3× with reconn_sz=16 only vs reconn_sz=16 + `input_shuffle=True`. Separates "bigger R" from "shuffle helps". | After FineWeb runs settle. Or now — they're independent. |

To write a new sweep, copy `lr_prism_3x.sh`, change the loop variable,
and update the `out_dir` / `run_name` template. The pattern:

```bash
sbatch --time=$WALLCLOCK --gpus-per-node=$GPUS_PER_NODE \
    --export=ALL,CONFIG=$CONFIG,EXTRA="--<knob>=$val --out_dir=... --run_name=..." \
    submit.sh
